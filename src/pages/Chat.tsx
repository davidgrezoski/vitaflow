...
const Chat = () => {
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-180px)] relative">
      {!hasApiKey && (
        <div className="mx-4 mt-10 mb-4 bg-amber-50 dark:bg-amber-900/20 p-4 rounded-2xl text-amber-800 dark:text-amber-200 text-xs font-bold border border-amber-200 dark:border-amber-800 flex gap-2 items-center">
          <AlertCircle size={16} />
      ...
      {error && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 bg-red-50 dark:bg-red-900/90 text-red-600 dark:text-red-200 px-4 py-2 rounded-full text-xs font-bold shadow-lg border border-red-100 dark:border-red-800 animate-fade-in">
          {error}
        </div>
      )}

      <div className="flex-1 overflow-y-auto px-2 space-y-6 pb-20 pt-4">
        ...
        {chatHistory.map((msg) => (
          ...
          >
            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm border border-transparent ${
              msg.role === 'user' ? 'bg-gray-900 dark:bg-white text-white dark:text-black' : 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-900/30'
            }`}>
            ...
            <div className="flex flex-col gap-1 max-w-[85%]">
              ...
              <span className={`text-[10px] font-bold uppercase tracking-wider mb-1 ${
                msg.role === 'user' ? 'text-right text-gray-400 dark:text-zinc-600' : 'text-left text-emerald-600 dark:text-emerald-500'
              }`}>
              ...
              <div className={`p-4 rounded-2xl text-sm shadow-sm leading-relaxed ${
                ...
                {msg.role === 'user' ? (
                  ...
                  <div className="prose prose-sm max-w-none prose-emerald dark:prose-invert prose-p:my-1 prose-ul:my-1 prose-li:my-0">
      ...
      <div className="absolute bottom-4 left-0 w-full px-2">
        <form onSubmit={handleSend} className="flex gap-2 items-center bg-white dark:bg-zinc-900 p-2 pl-4 rounded-full border border-gray-200 dark:border-zinc-800 shadow-lg shadow-gray-200/20 dark:shadow-black/50 focus-within:border-emerald-500/50 focus-within:ring-2 focus-within:ring-emerald-500/20 transition-all relative">
          ...
          <button 
            ...
            className="w-10 h-10 bg-emerald-500 text-white rounded-full flex items-center justify-center hover:bg-emerald-600 disabled:opacity-50 disabled:bg-gray-300 dark:disabled:bg-zinc-800 transition-all active:scale-90 shrink-0"
          >
            <Send size={18} />
          </button>
        </form>
